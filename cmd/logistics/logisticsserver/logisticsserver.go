...
func main() {
	...
	log := config.SetUpLogging(logOpt.LogisticsPath)
	...
	pwd, _ := os.Getwd()
	logisticsservices.StartLogisticsServer(log, false, pwd, dbOpt, redisOpt, mailerOpt, grpcServerOpt, jwtOpt, oauthOpt, userOpt, uptraceOpt, dbService, redisService, mailerService)
}
