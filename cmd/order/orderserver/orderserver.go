...
func main() {
	...
	log := config.SetUpLogging(logOpt.OrderPath)
	...
	pwd, _ := os.Getwd()
	orderservices.StartOrderServer(log, false, pwd, dbOpt, redisOpt, mailerOpt, grpcServerOpt, jwtOpt, oauthOpt, userOpt, uptraceOpt, dbService, redisService, mailerService)
}
