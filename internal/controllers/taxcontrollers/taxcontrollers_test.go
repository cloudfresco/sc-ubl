...
import (
	...
	taxservices "github.com/cloudfresco/sc-ubl/internal/services/taxservices"

	taxworkers "github.com/cloudfresco/sc-ubl/internal/workers/taxworkers"
...
func TestMain(m *testing.M) {
	...
	logTax = config.SetUpLogging(logOpt.TaxPath)
	...
	pwd, _ := os.Getwd()
	go taxservices.StartTaxServer(logTax, true, pwd, dbOpt, redisOpt, mailerOpt, grpcServerOpt, jwtOpt, oauthOpt, userOpt, uptraceOpt, dbService, redisService, mailerService)
	...
	go taxworkers.StartTaxWorker(log, true, pwd, grpcServerOpt, configFilePath)
